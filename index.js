...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
//The function 'nested Function' can access the varuable internal' because a function has the ability to access a variable outside of its scope, but more importantly, both functions reside withing the scope of myFunction.
...
const summ = numbers.reduce((currTotal, num) => {
  return num.numb + currTotal
}, 0)
...
const zooAnimals = [
 ...
 function lowerCaseNames(){
  const smolNames = zooAnimals.map((item)=> {
...
lowerCaseNames()
 ...
 function lowPopulationAnimals(){
  const anim = zooAnimals.filter((item) => {
...
lowPopulationAnimals()
  ...
  */
  ...
  function USApop(){
    const total = zooAnimals.reduce((currTotal, item) => {
      return item.population + currTotal
    }, 0)
